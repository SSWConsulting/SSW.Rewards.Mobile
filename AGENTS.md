...
- **Setup**: `aspire run` (from the repo root — `.aspire/settings.json` targets `src/AppHost`; .NET Aspire orchestrates SQL + Azurite + WebAPI + AdminUI; secrets prompted once into the AppHost user-secrets). See [Aspire-Local-Dev.md](_docs/Aspire-Local-Dev.md).
