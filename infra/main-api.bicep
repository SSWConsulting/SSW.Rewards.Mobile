...
module keyVault 'modules-api/keyvault.bicep' = {
  name: 'kv-${now}'
  params: {
...
module sqlServer 'modules-api/sql.bicep' = {
  name: 'sql-${now}'
  params: {
    projectName: projectName
    environment: environment
...
module appService 'modules-api/webapp.bicep' = {
  name: 'api-${now}'
  params: {
...
module kvWebAppRoleAssignment 'modules-api/add-kv-role-assignment.bicep' = {
  name: 'kvra-${now}'
  params: {
    keyVaultName: keyVault.outputs.keyVaultName
...
module storage 'modules-api/storage.bicep' = {
  name: 'st-${now}'
  params: {
    environment: environment
    location: location
...
module notificationhub 'modules-api/notificationhub.bicep' = {
  name: 'nh-${now}'
  params: {
    environment: environment
    location: location
